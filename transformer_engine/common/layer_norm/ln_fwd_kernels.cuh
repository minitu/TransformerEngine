...
template<typename Ktraits>
__global__ __launch_bounds__(Ktraits::THREADS_PER_CTA)
void bias_add_ln_fwd_tuned_kernel(BiasAddFwdParams params) {
    enum { ROWS_PER_CTA = Ktraits::ROWS_PER_CTA };
    enum { WARPS_N = Ktraits::WARPS_N };
    enum { WARPS_M = Ktraits::WARPS_M };
    enum { THREADS_PER_ROW = Ktraits::THREADS_PER_ROW };
    ...
    for ( int it = 0; it < LDGS; ++it ) {
        gamma[it].load_from(params.gamma, idx);
        beta[it].load_from(params.beta, idx);
        bias[it].load_from(params.bias, idx);
        idx += VEC_COLS_PER_LDG;
    }
    ...
    for ( int row = r; row < params.rows; row += params.ctas_per_col * ROWS_PER_CTA ) {
        ...
        for ( int it = 0; it < LDGS; it++ ) {
            x[it].load_from(params.x, idx);
            residual[it].load_from(params.residual, idx);
            #pragma unroll
            for ( int jt = 0; jt < NUM_ELTS; jt++ ) {
                // Add bias and residual to input
                compute_t bias_ij = bias[it].data.elt[jt];
                compute_t residual_ij = residual[it].data.elt[jt];
                compute_t x_ij = compute_t(x[it].data.elt[jt]);
                compute_t x_ba_ij = x_ij + bias_ij + residual_ij;
                xf[it * NUM_ELTS + jt] =  x_ba_ij;
                ba_out[it].data.elt[jt] = input_t(x_ba_ij);
            }
            ba_out[it].store_to(params.ba_out, idx);
            idx += VEC_COLS_PER_LDG;
        }
